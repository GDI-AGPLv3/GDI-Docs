...
Las respuestas del Gateway nunca exponen URLs directas de Cloudflare R2 (`pdf_url`, `signed_pdf_url`). Estos campos se eliminan recursivamente antes de devolver datos al cliente MCP (GDI-520: `signed_pdf_url` dejo de existir en el REST tambien — queda la key en `_STORAGE_URL_KEYS` como defensa barata).
